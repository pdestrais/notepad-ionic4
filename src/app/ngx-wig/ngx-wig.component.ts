...
import { NgxWigToolbarService, TButton } from '../services/ngx-wig-toolbar.service';

@Component({
  selector: 'ngx-wig',
  templateUrl: './ngx-wig.html',
  styleUrls: ['./ngx-wig.scss'],
  providers: [
  ...
  switch (command) {
      case 'createLink':
                ...
                if (selection === '') 
                  ...
                  document.execCommand('insertText', false, '['+options+']('+options + ')');
                else
                  document.execCommand('insertText', false, '['+selection+']('+options + ')');
                break;
      case 'insertImage':
                options = window.prompt('Please enter the URL', 'http://');
                if (!options) {
